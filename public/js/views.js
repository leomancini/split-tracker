...
function addExpenseView(gid, ex){
  ...
  if(members.length >= 2){
    ...
    h += '<option value="uneven">Split by amount</option>';
    ...
    members.forEach(function(m, idx){
      ...
      h += '<div class="exp-participant-row" style="display:flex;align-items:center;gap:0.625rem;padding:0.75rem;margin:0'+borderStyle+'">'
        ...
        + '<span class="exp-uneven-prefix" style="display:none;font-size:0.9375rem;color:var(--gray-900);margin-right:-0.5rem">$</span>'
        + '<input type="number" class="exp-uneven-amt" data-member-id="'+m.id+'" step="0.01" min="0" placeholder="0.00"'
        + ' style="display:none;width:72px;height:1.4rem;text-align:left;padding:0 0.375rem;border:1px solid var(--gray-300);border-radius:4px;font-size:16px">'
        + '<span class="exp-pct-preview" style="display:none;font-size:0.9375rem;color:var(--gray-500);white-space:nowrap"></span>'
        ...
        + '<span class="exp-pct-sign" style="display:none;font-size:0.9375rem;color:var(--gray-900);margin-left:-0.5rem">%</span>'
        + '</div>';
    });
    h += '</div>'
      + '</div>';
...
function itemDetailView(gid, ex, isOwner){
  ...
  if(ex.settled_with){
    ...
    if(ex.split_type === 'full'){
      ...
      h += '<div class="info-row" style="border-bottom:none; padding-bottom:10px"><span class="info-label">Split</span><span class="info-value">'+(pcts ? 'By percentage' : 'By amount')+'</span></div>';
